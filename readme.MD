...
如果你看不懂下面这句代码，先看看<a href='#9.8.2'>这里</a>。
...
``` 
[]=操作符具体怎么实现的我还不知道，不过我知道这句代码是相当于java map中的put方法，如：
```dart
  Map<String, int> hist = {
    'a': 23,
    'b': 100,
...
void main() {
  ...
  //[]=也是一个操作符，它必须返回一个void，可以见Map的[]=操作符，相当于java Map中的put方法，不过它的用法我还没弄太明白。
  assert(v[w] == Vector(4, 5));
}
```
