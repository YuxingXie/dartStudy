...
```
##### 4.6.7 计数器(Generators)
当您需要惰性地生成一系列值时，请考虑使用生成器函数。Dart内置支持两种生成器函数:
* 同步生成器:返回一个Iterable对象。
* 异步生成器:返回一个流对象。

要实现同步生成器函数，将函数体标记为sync*，并使用yield语句来传递值(类似于return):
```dart
...
```
要实现异步生成器函数，请将函数体标记为async*，并使用yield语句来传递值(类似于return):
```dart
