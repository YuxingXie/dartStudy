...
洗菜-->切菜-->炒菜-->上菜
```
我们把这四个步骤认为是同步操作，即程序是按照顺序一步步执行的。
...
饭店就产生了一个需要处理的炒菜事件，如果有很多客人点菜，就产生了一个事件队列，厨师不停的炒菜，客人不停的点菜，便形成了一个事件循环。

这一切都发生在一个isolate里，就是一条线，它没有任何需要和其它isolate互动的地方，这条线也就是main函数执行的这条线，我就叫它主线吧。
